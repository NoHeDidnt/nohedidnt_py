...
if not isinstance(pkg_info, tuple) or not len(pkg_info) == 2:
    raise ValueError("Invalid pkg_info")

PKG_DIR, PKG_NAME = pkg_info
...
pkg_env_info = nhd_pkg_env_info(ENV_FILE)
if not isinstance(pkg_env_info, tuple) or not len(pkg_env_info) == 2:
    raise ValueError("Invalid pkg_env_info")

PKG_ABBR, PKG_AUTHOR = pkg_env_info
