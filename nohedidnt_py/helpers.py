...
def nhd_timer(
    name: str = const.PKG_ABBR, text: str = const.MESSAGES.get("timer", const.PKG_TITLE), logger: OptCallable = nhd_debug, start: bool = True
) -> Timer:
...
def color_from_value(value: AnyVal) -> OptColor:
    ...
    if "#" not in value and 2 < len(value) <= 6:
        value = f"#{value}"
